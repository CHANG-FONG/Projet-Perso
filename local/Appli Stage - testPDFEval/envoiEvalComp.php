...
function envoiresultat($SQLRow, $idstage, $BDDConn, $aEnvoyer)	{
	$queryid = 'SELECT COALESCE (max(id),0)+1 as ids_e FROM contenirComp ';
		...
		$SQLStmt->bindValue(':idniveauCompe', $aEnvoyer);
		if (!$SQLStmt->execute()) {
			var_dump($SQLStmt->errorInfo());
		}
}

envoiresultat($SQLRow, $idstage, $BDDConn, $Bl11);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl12);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl13);

envoiresultat($SQLRow, $idstage, $BDDConn, $Bl21);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl22);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl23);

envoiresultat($SQLRow, $idstage, $BDDConn, $Bl31);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl32);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl33);

envoiresultat($SQLRow, $idstage, $BDDConn, $Bl41);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl42);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl43);

envoiresultat($SQLRow, $idstage, $BDDConn, $Bl51);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl52);
envoiresultat($SQLRow, $idstage, $BDDConn, $Bl53);
